...
public class PostChecklistsCidTasksLid extends Command {
    ...
    public Object execute(HashMap<String, String> params, Connection con) throws Exception {
        ...
        if (!rs.next()) {
            ...
            String isClosed = "0";
            if (params.get("isClosed").equals("true"))
                isClosed = "1";
            ...
            s = "select * from checklist_task where Cl_task_closed = 0 and Cl_id = " + cid;
            ps = con.prepareStatement(s);
            rs = ps.executeQuery();
            ...
            if (!rs.isBeforeFirst()) {
                ...
                s = "update checklist set Cl_closed = 0 where Cl_id = " + params.get("{cid}");
                ps = con.prepareStatement(s);
                ps.execute();
            }
            return "Success";
